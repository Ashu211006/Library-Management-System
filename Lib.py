...
c1.execute("create table users (username varchar(30), passw 
varchar(30))") 
c1.execute("insert into users values('Ashutosh','12345')") 
db1.commit() 
c1.execute("create table member(mid varchar(20) primary 
key,name varchar(50),email varchar(50), phone varchar(20))") 
c1.execute("create table book(bookid varchar(20) primary key,title 
varchar(50), author varchar(50), publisher varchar(50), cost 
integer)") 
c1.execute("create table issue(mid varchar(20), bookid 
varchar(20), dateofissue date)") 
c1.execute("create table issuelog(mid varchar(20), bookid 
varchar(20), dateofissue date, dateofreturn date)") 
...
def returnBook(): 
    ...
    mid = input("Enter the Member id : ") 
    q ="select dateofissue from issue where bookid='" + bid +"' and 
mid='" + mid +"'" 
    cursor1 = db1.cursor() 
    cursor1.execute(q) 
    res = cursor1.fetchall() 
    if len(res)== 0: 
        ...
        q = "delete from issue where bookid='" + bid + "' and mid='" + 
mid + "'" 
        cursor1.execute(q) 
        db1.commit() 
